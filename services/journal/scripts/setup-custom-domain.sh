...
# Script to configure custom domain for Journal API Gateway
# Usage: ./scripts/setup-custom-domain.sh

set -e

REGION="us-east-2"
API_DOMAIN="journal.demo.balansi.me"
HOSTED_ZONE_NAME="balansi.me"  # Adjust if your hosted zone is different

echo "🔧 Configuring custom domain for Journal API Gateway..."
...
# Try to find certificate for journal.demo.balansi.me, *.demo.balansi.me, or *.balansi.me
CERT_ARN=$(aws acm list-certificates \
    --region "$REGION" \
    --query "CertificateSummaryList[?DomainName=='$API_DOMAIN' || DomainName=='*.demo.balansi.me' || DomainName=='*.$HOSTED_ZONE_NAME'].CertificateArn" \
    --output text | head -1)
...
if [ -z "$CERT_ARN" ] || [ "$CERT_ARN" == "None" ]; then
    ...
    if [ -z "$CERT_ARN" ] || [ "$CERT_ARN" == "None" ]; then
        ...
        echo "   Please create a certificate manually in ACM:"
        ...
        echo "   Configuring DNS validation..."
        ...
        VALIDATION_RECORDS=$(echo "$CERT_DETAILS" | jq -r '.Certificate.DomainValidationOptions[]? | select(.ValidationStatus != "SUCCESS") | .ResourceRecord')
...
EOF
                ...
                if [ "$CERT_STATUS" != "ISSUED" ]; then
                    ...
                    echo "   Please check the certificate status manually and run this script again"
                    echo "   Certificate ARN: $CERT_ARN"
                fi
            else
                echo "   ⚠️  Could not find Route53 hosted zone. Please add DNS validation record manually:"
...
echo "   Target Domain: $TARGET_DOMAIN"
echo ""
echo "🔗 Your Journal API will be available at:"
